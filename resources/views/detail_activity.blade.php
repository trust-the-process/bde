        ...
        <div class="container"> 
            <div class="row no-gutters slider-text align-items-center justify-content-center">
                <div class="col-md-9 ftco-animate text-center">
                    <p class="breadcrumbs"><span class="mr-2"><a href="/activity">Home</a></span> <span>Blog</span></p>
                    <h1 class="mb-0 bread">{{ $activity->title }}</h1>
                </div>
            </div>
        </div>
    </div>
	...
	<section class="ftco-section ftco-degree-bg">
        <div class="container">
          <div class="row">
            ...
            <h2 class="mb-3 mt-5">#{{ $activity->id }}. {{ $activity->title }}</h2>
              <p> {{ $activity->description }}</p>
              <p>
                <img src="{{asset('uploads/product/'.$activity->image) }}" alt="" srcset=""  class="img-fluid" > 
              </p>
              <p>{{ $activity->text }} </p>
              <div class="tag-widget post-tag-container mb-5 mt-5">
                <div class="tagcloud">
                  <a href="#" class="tag-cloud-link">{{ $activity->mot_cles }}</a>
                </div>
              </div>
              
              <div class="about-author d-flex p-4 bg-light">
                @php
                    $user = DB::select('select * from users where id = '.$activity->provider.'' ) ;
                @endphp
                <div class="bio align-self-md-center mr-4">
                  <img src="{{asset('../1.png')}}" alt="Image placeholder" class="img-fluid mb-4">
                </div>
              ...
              <div class="pt-5 mt-5">
                @php
                    $comment = DB::select('select * from comment where type = "activity" and provider = '.$activity->id.' ' )
                @endphp
                ...
                <ul class="comment-list">
                  ...
                  <li class="comment">
                    <div class="vcard bio">
                      <img src="{{asset('../1.png')}}" width="222px" height="122px" alt="Image placeholder">
                    </div>
                ...
                <div class="comment-form-wrap pt-5">
                  ...
                  <form action="{{ route('form_add_comment') }}" method="POST" enctype="multipart/form-data" class="bg-white p-5 contact-form" role="form">
                    ...
                    <input type="hidden" name="motif" value="activity">
                    <input type="hidden" name="type" value="activity">
                    <input type="hidden" name="provider" value="{{ $activity->id }}">
            ...
            <div class="col-lg-4 sidebar ftco-animate">
              ...
              <div class="sidebar-box ftco-animate">
                <h3 CLASS="heading">Recent activitys</h3>
                @php
