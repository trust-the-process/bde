                            ...
                            <div class="col-md-12 d-flex ftco-animate">
                                <div class="blog-entry align-self-stretch d-md-flex">
                                    <a href="/detail_activity/{{ $item->id }}" class="block-20" style="background-image: url('{{asset('uploads/product/'.$item->image) }}');">
                                        <img src="{{asset('uploads/product/'.$item->image) }}" alt="" style="max-height: 29em" srcset="">
                                    </a>
                                    <div class="text d-block pl-md-4">
                                        <div class="meta mb-3">
                                        <h2>{{ $item->title }}</h2>
                                        <div><strong >{{ $item->club }}</strong></div>
                                        <div><h6>Par : 
                                            @php
                                        ...
                                        <h5 class="heading"><strong>Du {{ (date('d F Y', strtotime($item->created_at))) }} au {{ (date('d F Y', strtotime($item->updated_at))) }}</strong></h5>
                                        <p>{{ $item->description }}</p>
                                        <p><a href="/detail_activity/{{ $item->id }}" class="btn btn-success py-2 px-5">Read more</a></p>
                                    </div>
                                </div>
                            </div>
                            @endforeach
